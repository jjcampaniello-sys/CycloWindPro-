...
function calculateWindScore(latlngs, estUneRueAbritee = false){
    ...
    for(let i = 0; i < latlngs.length - 1; i++){
        ...
        // 🔥 BONUS ANALYSE : Si l'adresse sélectionnée est identifiée comme une rue résidentielle abritée
        if (estUneRueAbritee) {
            cost = cost * 0.7; // Réduction automatique de 30% de l'effort face au vent grâce aux bâtiments
        }
...
// Correction du padding manquant généré par le système automatique
function monPaddingSecurise() {
    return [50, 50];
}
...
async function getRoute(){
    ...
    // Analyse du type d'adresse sauvegardée par search.js pour appliquer le bonus
...
function startNavigation() {
    ...
    if (!window.isNavigating) {
        ...
        if (windInfoPanel) {
            window.classList.remove("nav-hidden");
        }
    ...
    // 2. Détection des parcs et forêts (surfaces en terre/gravier)
    ...
    for(let i = 0; i < latlngs.length - 1; i++){
        ...
        // Application automatique des bonus d'obstacles physiques
        if (forestSegments.has(i)) {
            cost = cost * 0.5; // Les arbres coupent le vent de 50%
        } 
        else if (residentialSegments.has(i)) {
            cost = cost * 0.7; // Les maisons coupent le vent de 30%
        }
...
async function getRoute(){
    ...
    // 🔥 INJECTION SÉCURISÉE : Transmission des données d'obstacles cachées de l'API au calcul
    ...
    // 🔥 REPARÉ ET FERMÉ : Le zoom automatique s'applique sans encombre
    if (latlngsNormal && latlngsNormal.length > 0) {
        const bounds = L.latLngBounds(latlngsNormal);
...
// 🔥 REPARÉ ET AJOUTÉ EN ENTIER : Fonction de navigation et masquage du bandeau blanc au clic
function startNavigation() {
